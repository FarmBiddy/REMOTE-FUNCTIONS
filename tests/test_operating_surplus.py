...
def test_loan_repayments_do_not_change_operating_surplus() -> None:
    """Finance may change; operating costs, surplus, and margin must not (B5.3)."""
    ...
    assert zero["profit"]["margin"] == twelve["profit"]["margin"] == twenty["profit"]["margin"]
    assert (
        zero["profit"]["margin_pct"]
        == twelve["profit"]["margin_pct"]
        == twenty["profit"]["margin_pct"]
    )
...
@pytest.mark.parametrize("cost_name", OPERATING_COST_CATEGORIES)
def test_each_operating_cost_contributes_to_total_and_surplus(cost_name: str) -> None:
    """Every catalogue line must affect costs.total and Operating Surplus (B5.1)."""
    base = pl_summary(**MILK_ONLY)
    assert base["costs"]["total"] == 0
    base_surplus = base["profit"]["net"]

    updated = pl_summary(**{**MILK_ONLY, cost_name: 1_000})
    assert updated["costs"]["lines"][cost_name] == 1_000
...
def test_omitted_optional_operating_costs_default_to_zero() -> None:
    result = run_function("pl.summary", MILK_ONLY)["result"]
