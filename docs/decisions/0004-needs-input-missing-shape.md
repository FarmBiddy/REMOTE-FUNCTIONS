...
- Units are defined in `FIELD_UNITS` in `farm_functions/schemas.py`, derived from `INPUT_FIELD_METADATA`.
...
- New input fields must register metadata in `INPUT_FIELD_METADATA` (unit, required, minimum, description). `FIELD_UNITS` is derived from that list.
